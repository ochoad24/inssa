<template>
  <div v-if="showItem('Ver lista de cajas')">
    <vx-card title="Listado de cajas">
      <div class="flex -mx-2">
      ...
      <div id="data-list-list-view" class="data-list-container">
        <vs-table :data="accounts">
          <template slot="thead">
            ...
            <vs-th sort-key="in_charge.name">Usuario encargado</vs-th>
            ...
            <vs-tr :key="account.id" v-for="account in data">
              ...
              <vs-td>
                <vs-chip :color="'primary'" class="product-order-status">
                  {{ account.in_charge.name }}</vs-chip
                >
              </vs-td>
              <vs-td>
                <vs-chip :color="'primary'" class="product-order-status">
              ...
              <vs-td class="whitespace-no-wrap">
                ...
                <feather-icon
                  ...
                  v-if="showItem('Editar caja')"
                  @click="updateAccount(account.id)"
                />
                <feather-icon
                  icon="TrashIcon"
                  svgClasses="w-5 h-5 hover:text-danger stroke-current"
                  v-if="showItem('Eliminar caja')"
...
export default {
  ...
  methods: {
    newAccount() {
      this.$router.push({ path: `/cajas/nuevo` });
    },
    showItem(namePermit) {
    ...
    deleteAccount() {
      ...
      vm.$vs.loading();
      axios
        .delete(`/api/cashregister/${vm.idAccount}`)
        .then((response) => {
          vm.$vs.loading.close();
          vm.$vs.notify({
            title: 'Listo',
    ...
    async getAccounts() {
      ...
      const status = this.showItem('Ver solo mi caja');
      console.log(status);

      await axios
        .get(
          `/api/cashregister?page=${vm.pagination.current}&search=${
            vm.filterSearch
          }&admin=${status}&user=${
